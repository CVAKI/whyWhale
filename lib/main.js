...
const { loadConfig, saveConfig, loadMemory, saveMemory, parseMemoryBlocks, updateMemory, sanitizeMemory, buildMemoryContext, SKILL_REGISTRY, loadSkills, saveSkill, buildSkillsContext, HOME_DIR, CONFIG_PATH, SKILLS_DIR } = require('./config');
const { PROVIDERS, ollamaAvailable, ollamaModels, ollamaPull, ollamaInstall, ollamaStart, fetchLiveModels, callAI } = require('./providers');
const { formatMD, renderPS1, printBanner, spinner } = require('./render');
const { CWD, safePath, ensureDirForFile, writeFileSafe, readFileSafe, treeDir, lsDir, formatSize, parseFileBlocks, parseRunBlocks, applyFileBlocks, printFileResults, scanFolder, buildFolderContext } = require('./filesystem');
...
async function main() {
  ...
  let folderCtx  ='';
  // When the AI is mid-task in agent mode (wrote files this turn), suppress
  // auto-switching away so follow-up turns don't lose write permissions.
  ...
  if (providerKey==='ollama') {
    ...
    if (!availModels.length) {
      ...
      // Models are installed — badge any that are in the top-coding list
      availModels = availModels.map(m => ({
        ...m,
  ...
  while (!connected) {
    if (!modelId) {
      console.log('');
      if (providerKey==='ollama') {
        // Sort: coding models first
        const codingModels = availModels.filter(m=>m.coding);
        const otherModels  = availModels.filter(m=>!m.coding);
        let idx=0;
        ...
        // Rebuild flat ordered list so index picks are correct
        availModels=[...codingModels,...otherModels];
      } else {
  ...
  async function _handleLine(raw) {
    ...
    if (text.startsWith('!')) {
      ...
      // Fix: rewrite Unix background operator `&` to Windows equivalent on Win32
      let cmdToRun = cmd;
    ...
    if (text==='/provider') {
      ...
      const olOk=await ollamaAvailable();
      console.log('  '+kp('[4]')+' Ollama'+(providerKey==='ollama'?kp(' ◀'):'')+' '+(olOk?kp('● running'):rf('○ not running — will auto-install if selected')));
    ...
    if (text.startsWith('/memory')) {
      ...
      if (!arg||arg==='show') {
        ...
        sanitizeMemory(mem); saveMemory(mem);
        const removed=before-mem.facts.length;
        console.log('\n  '+kp('✔ Memory cleaned — ')+sd(removed+' garbage fact'+(removed!==1?'s':'')+' removed')+ab(', '+mem.facts.length+' clean facts remain'));
      } else if (arg==='clear') {
...
<\\/html>`;
    ...
    if (text.startsWith('/analyse ')||text.startsWith('/analyze ')) {
      // Auto-switch to Review mode for file analysis
      if (mode !== 'review') {
        mode = 'review';
        saveConfig({provider:providerKey,apiKey,model:modelId,mode,autoTest,autoScan});
        statusRef.mode = mode;
    ...
    // automatically and announce what changed.
    {
      ...
      // Don't auto-switch away from agent if a file-writing task is still in progress
      if (!agentTaskActive) {
    ...
    try {
      ...
      // Save memory blocks — skip if key+value already identical (prevents per-turn spam)
      const memBlocks=parseMemoryBlocks(reply);
      const newMemBlocks = memBlocks.filter(mb => {
        const existing = mem.facts.find(f => f.key === mb.key);
        return !existing || existing.value !== mb.value;
      });
      if (newMemBlocks.length){
        updateMemory(mem, newMemBlocks); saveMemory(mem);
      ...
      if (blocks.length) {
        if (mode==='agent') {
          ...
          agentTaskActive = true; // stay in agent mode for follow-up turns
        } else {
          console.log('\n  '+vt('AI wants to create/modify '+blocks.length+' file(s):'));
          blocks.forEach(bk=>console.log('  '+ab('  → ')+sd(bk.relPath)));
          console.log('  '+rf('  ⚠  You are in ')+MODES[mode].colorFn(MODES[mode].icon+' '+MODES[mode].name)+rf(' mode — file writes are OFF by default. Use /mode agent to auto-apply.'));
        ...
        // (keeps agent mode across multi-turn sequences of run-only turns)
        if (!parseRunBlocks(reply).length) {
          agentTaskActive = false;
        }
      }

      // ── Execute @@RUN: blocks ─────────────────────────────────────────────
      // The AI emits these to run real commands instead of simulating output.
      const runCmds = parseRunBlocks(reply);
      if (runCmds.length) {
        // ── Auto-start server if it is not running ──────────────────────────
        // Background servers launched by Start-Process (Windows) don't survive
        // across whyWhale turns. Before running any client commands, check if
        // port 3000 is already open — if not, find server.js and start it.
        // Probe port with TCP — returns true if something is listening
        ...
        // Only auto-start if the command is a client command (not taskkill/netstat/curl etc.)
        ...
        async function ensureServer() {
          ...
          const candidates = [
            _path2.join(CWD(), 'server.js'),
            _path2.join(CWD(), 'src', 'server.js'),
            _path2.join(CWD(), 'app.js'),
          ];
          const serverFile = candidates.find(f => _fs2.existsSync(f));
          if (!serverFile) return false;
          const rel = _path2.relative(CWD(), serverFile);
          console.log('  ' + wh('⟳') + ' ' + ab('Auto-starting server: ') + tl(rel));
          await runShell('node "' + serverFile + '" &');
          // Poll up to 5s for port to open (25 x 200ms), check both hosts
          for (let i = 0; i < 25; i++) {
        ...
        for (let cmd of runCmds) {
          ...
          // Models sometimes emit bare JS function names (listTasks, addTask…)
          // instead of shell commands. Detect and skip them gracefully.
          const firstToken = cmd.trim().split(/\s+/)[0];
          const looksLikeShell = /^[a-z]:[\\//]/i.test(firstToken) ||  // C:\ path
                                 /^[\.~\/\\]/.test(firstToken) ||     // relative path
                                 /\.(js|py|sh|cmd|bat|exe|ps1)$/i.test(firstToken) || // file ext
          ...
          // AI often appends & to client commands — on Windows this puts them
          // in background and swallows stdout. Only server.js/app.js need &.
          ...
          // If this command explicitly starts the server in background
          ...
          // Only probe for client commands (node client.js ...) — not for
          // taskkill, curl, netstat, or other shell utilities.
          if (!serverEnsured && !isServerCmd && isClientCmd(cmd)) {
            const alreadyUp = await probePort(3000);
            if (alreadyUp) {
              serverStarted = true; // server is running, mark it
            } else {
              const autoStarted = await ensureServer();
          ...
          if (out.includes('ECONNREFUSED')) {
            ...
            serverStarted = true;
            serverEnsured = true;
            // Poll both hosts before retrying
            let ready = false;
          ...
          // If no output and exit 0, retry for any node client command —
          // serverStarted may be false on first run but server is still up.
        ...
        if (runOutputLines.length) {
          messages.push({ role:'user', content:'@@RUN results:\n```\n'+runOutputLines.join('\n')+'\n```' });
          messages.push({ role:'assistant', content:'Run output received.' });
        }
      }
