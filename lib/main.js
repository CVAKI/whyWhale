...
async function main() {
  ...
  const memCtx   =buildMemoryContext(mem,CWD());
  const skillsCtx=buildSkillsContext(skills);
  ...
  rl.on('line', async raw => {
    ...
    if (text.startsWith('/mode')) {
      ...
      } else { console.log('\n  '+dg('Unknown mode. Options: ')+Object.keys(MODES).join(' · ')); }
      prompt(); return;
    }
    ...
    if (text==='/export') {
      const lines2=['# whyWhale Export','> '+new Date().toLocaleString()+' · Mode: '+mode+''];
      messages.forEach(m=>{if (m.role==='system') return; lines2.push(m.role==='user'?'## You':'## whyWhale',m.content,'');});
      const fp=path.join(HOME_DIR,'whywhale_export_'+Date.now()+'.md');
      fs.writeFileSync(fp,lines2.join('\n'));
      console.log('\n  '+kp('✔ Exported → ')+ab(fp));
      prompt(); return;
    }
    ...
    if (text.startsWith('/write ')) {
      ...
      try {
        ...
        console.log('\n  '+wh('🐋 whyWhale')+'  '+ab(((Date.now()-t1)/1000).toFixed(1)+'s · '+totalTok.toLocaleString()+' tokens'));
        console.log(''); console.log(formatMD(reply));
