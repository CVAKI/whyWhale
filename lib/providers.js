...
// Auto-install Ollama based on the current OS.
// Streams the installer output directly to the user's terminal.
async function ollamaInstall() {
  const platform = process.platform;
  return new Promise((resolve, reject) => {
    let proc;
    if (platform === 'win32') {
      // Windows: download the official setup exe silently via PowerShell
      const ps = [
...
// Start the Ollama server in the background, then wait up to ~20 s for it to be ready.
async function ollamaStart() {
  const platform = process.platform;
  try {
    if (platform === 'win32') {
      // On Windows the installer registers a service / tray app; just try launching it.
  ...
  } catch(_) { /* ollama may already be starting */ }

  // Poll until the API responds (max 20 attempts × 1 s = 20 s)
  for (let i = 0; i < 20; i++) {
