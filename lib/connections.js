...
async function setupWhatsApp(ctx) {
  ...
  const tryLoad = (bustCache = false) => {
    // Only bust the require cache after a fresh npm install.
    // Busting on every /wp call resets all module-level state (including
    // _startupSent), which caused duplicate startup DMs on every reconnect.
  ...
  try {
    ...
    // Bust cache here only — deps were just installed so a fresh load is needed
  ...
  return new Promise((resolve) => {
    startWhatsApp({
      onConnected: async () => {
        ...
        // Bust the WA module cache after a successful setup so that any
        // subsequent require() of connections/whatsapp/index.js (e.g. in the
        // stress-test suite) returns a fresh instance with _activeSock = null.
        // The live connection is unaffected — ctx.waClient and
        // ctx._waSendFarewell hold direct references to the current module's
        // exported functions and continue to work normally.
        try {
