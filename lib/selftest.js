...
function runShell(cmd) {
  return new Promise(res => {
    ...
    // On Windows, trailing & (background job) is a PowerShell syntax error.
    // Rewrite to Start-Process so servers can start in the background.
    if (process.platform === 'win32' && /&\s*$/.test(cmd.trim())) {
      const base  = cmd.trim().replace(/&\s*$/, '').trim();
      ...
      if (parts) {
        ...
        actualCmd = `powershell -NoProfile -Command "${pscmd}"`;
      }
    }

    exec(actualCmd, {timeout:30000, cwd:CWD(), shell:true}, (err,stdout,stderr) => {
      res({stdout:stdout||'', stderr:stderr||'', code:err?(err.code||1):0});
    });
  });
}
