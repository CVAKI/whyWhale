...
const BASE_DIR    = path.join(HOME_DIR, '.whyWhale');
...
function parseMemoryBlocks(text) {
  // Guard against non-string input (null, undefined, numbers, arrays, objects)
...
function sanitizeMemory(mem) {
  ...
  // Ensure facts is always a valid array even if the JSON on disk was corrupted
  if (!Array.isArray(mem.facts)) mem.facts = [];

  // Ensure sessionSummaries is always a valid array
  if (!Array.isArray(mem.sessionSummaries)) mem.sessionSummaries = [];

  // 1. Filter out noise/garbage facts
  mem.facts = mem.facts.filter(f => {
    if (!f || typeof f !== 'object') return false;  // skip null/non-object entries
