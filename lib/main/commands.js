...
async function handleShell(text, ctx) {
  ...
  // Detect Unix-only commands on Windows and give a helpful hint
  ...
  // Rewrite Unix background operator `&` to Windows equivalent on Win32
  let cmdToRun = cmd;
...
async function handleExit(text, ctx) {
  ...
  console.log(''); ctx.rl.close(); process.exit(0);
}
...
// ─── dispatchCommand ──────────────────────────────────────────────────────────
// Try each command in order. Returns true if the input was handled as a command.
async function dispatchCommand(text, ctx) {
  ...
  return false; // not a recognised command — pass to AI
}
...
async function handleConnection(text, ctx) {
  ...
  if (!sub || sub === 'list') {
    ...
    console.log('\n  ' + ab('─'.repeat(52)));
    console.log('  ' + ab('Type ') + sd('/connection <name>') + ab(' to set up.'));
  ...
  if (sub === 'disconnect') {
    ...
    if (!target || !CONNECTION_REGISTRY[target]) {
      console.log('\n  ' + dg('Usage: /connection disconnect <name>'));
      ctx.prompt(); return true;
    }
...
async function handleWa(text, ctx) {
  ...
  // ── /wa (no args) or /wa help → show usage ────────────────────────────────
  if (!args || args === 'help') {
    ...
    console.log('  ' + GL + '/wp                    ' + R + GR + '  — open WhatsApp setup' + R);
    console.log('  ' + DIV);
  ...
  if (args === 'status') {
    ...
    console.log('');
    if (ctx.waClient) {
      console.log('  ' + G + B + '● WhatsApp: connected' + R + GL + '  (client active this session)' + R);
    } else if (status?.connected) {
      console.log('  ' + GD + '● WhatsApp: saved' + R + GR + '  (credentials exist, not active — restart whyWhale to auto-connect)' + R);
    } else {
      console.log('  ' + '\x1b[38;5;203m' + '○ WhatsApp: not connected' + R);
      console.log('  ' + GR + 'Run ' + R + G + '/wp' + R + GR + ' to set up.' + R);
    }
    console.log('');
    ctx.prompt(); return true;
  }
  ...
  // ── /wa <number> <message> ────────────────────────────────────────────────
  // Check client is live
  if (!ctx.waClient) {
    const { getConnectionStatus } = require('../connections');
    const status = getConnectionStatus('whatsapp');
    console.log('');
    if (status?.connected) {
      console.log('  ' + '\x1b[38;5;226m' + '⚠ WhatsApp client is not active this session.' + R);
      ...
      console.log('  ' + GR + 'Run ' + R + G + '/wp' + R + GR + ' to set up WhatsApp first.' + R);
    }
    console.log('');
    ctx.prompt(); return true;
  }

  // Parse number + message
  const spaceIdx = args.indexOf(' ');
  if (spaceIdx === -1) {
    console.log('\n  ' + '\x1b[38;5;203m' + '✘ No message provided.' + R);
    console.log('  ' + GR + 'Usage: ' + R + G + '/wa <number> <message>' + R);
    console.log('  ' + GR + 'Example: ' + R + G + '/wa 919876543210 Hello!' + R + '\n');
    ctx.prompt(); return true;
  }

  let number  = args.slice(0, spaceIdx).trim().replace(/\D/g, ''); // strip non-digits
