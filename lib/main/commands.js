...
const { saveConfig, saveMemory, updateMemory, sanitizeMemory,
        SKILL_REGISTRY, saveSkill, HOME_DIR, SKILLS_DIR,
        parseMemoryBlocks }                                                   = require('../config');
...
const { CWD, safePath, ensureDirForFile, readFileSafe, treeDir, lsDir,
        formatSize, parseFileBlocks, applyFileBlocks,
...
async function handleShell(text, ctx) {
  ...
  // Both cmd.exe (\"  is not an escape) and PowerShell -Command ("..." with \"
  // inside it) mangle curl's JSON headers and body before curl ever sees them.
  // Fix: parse the curl command into an argv array here in Node, write the JSON
  // body to a temp file, then spawnSync('curl', argv) — zero shell involvement.
  if (process.platform === 'win32' && /^\s*curl\s/i.test(cmdToRun)) {
    // ── arg parser: handles "quoted strings" and \" inside them ────────────
    ...
    // ── if -d body present, write to temp file and replace with @file ───────
    const dIdx = argv.findIndex(a => a === '-d' || a === '--data' || a === '--data-raw');
    let tmpJson = null;
    if (dIdx !== -1 && argv[dIdx + 1] !== undefined) {
      const body = argv[dIdx + 1];            // already unquoted by parser above
    ...
    ctx.prompt(); return true;  // handled — skip the generic runShell path
  }
  ...
  // If the command runs a .js file and fails with "Cannot find module 'X'",
  // extract every missing package, npm install them all, then retry once.
  ...
  if (res.code !== 0 && isNodeRun && /Cannot find module/.test(combinedOut)) {
    ...
    if (toInstall.size) {
      ...
      console.log('  ' + wh('⬇') + ' ' + ab('Auto-installing missing module(s): ') + sd(pkgList.join(', ')) + ab('...'));
      const { exec: _exec } = require('child_process');
      const installed = await new Promise(resolve => {
        _exec('npm install ' + pkgList.join(' ') + ' --save',
          { timeout: 90000, cwd: require('../filesystem').CWD(), shell: true },
          err => {
      ...
      if (installed) {
        console.log('  ' + wh('⟳') + ' ' + ab('Retrying command after install...'));
...
// ─── /debug -fix ──────────────────────────────────────────────────────────────
// /debug -fix [file]   — scan, understand errors, auto-install deps, fix & run live
async function handleDebugFix(text, ctx) {
  const { callAI: _callAI } = require('../providers');
  const { scanFolder, buildFolderContext, CWD, parseFileBlocks, applyFileBlocks, printFileResults } = require('../filesystem');
  const { exec: _exec } = require('child_process');
  ...
  if (!targetFile) {
    // scanFolder returns [{path, size, ext}] — .path is the CWD-relative path
    // Detects ALL file types: code, data, docs, images, video, audio, archives
    ...
    const fs2 = require('fs'), pathM2 = require('path');
    const allFiles = scanned
      .filter(f => f.path && ALL_EXTS.has(pathM2.extname(f.path).toLowerCase()))
      .map(f => {
        const full2 = pathM2.resolve(CWD(), f.path);
        try { return { rel: f.path, ext: pathM2.extname(f.path).toLowerCase(), mtime: fs2.statSync(full2).mtimeMs }; }
        catch(_) { return { rel: f.path, ext: pathM2.extname(f.path).toLowerCase(), mtime: 0 }; }
      })
  ...
  const fs3 = require('fs'), pathM = require('path');
  const fullPath = pathM.resolve(CWD(), targetFile);
  ...
  // Pick the right interpreter for the file type
  const fileExt = pathM.extname(targetFile).toLowerCase();
  ...
  const RUNNERS = {
    ...
    '.go': 'go run',
    '.php': 'php',
    '.rs': null,   // compiled — AI fix only
    '.java': null, // compiled — AI fix only
    '.cs': null,
    '.swift': null,
    '.kt': null,
  ...
  const tryRun = () => new Promise(res => {
    if (!runner) {
      // Non-runnable (binary, doc, compiled) — report as "needs AI fix/analysis"
  ...
  while (round < MAX_ROUNDS) {
    ...
    // Auto-install missing modules (free, doesn't burn an AI round)
    ...
    if (toInstall2.size) {
      ...
      if (ok) { round--; continue; }  // free retry
    }
    ...
    const fileContent = fs3.readFileSync(fullPath, 'utf8');
    const fixPrompt = [
    ...
    if (blocks.length) {
      ...
      if (rawMatch) {
        fs3.writeFileSync(fullPath, rawMatch[1].trim(), 'utf8');
...
// ─── /token ───────────────────────────────────────────────────────────────────
// /token                    — show current limit + table
// /token -set-usage <n>     — set max_tokens for all AI calls
// /token -show              — same as /token
// Legacy aliases: /coding -set-token <n> / /coding -show still work
async function handleToken(text, ctx) {
  const args  = text.trim().split(/\s+/);
  // Detect command name to support both /token and legacy /coding
  const sub   = args[1]; // -set-usage | -show | -set-token (legacy)
  ...
  // ── /token -set-usage <n>  (also accepts legacy -set-token) ───────────────
  if (sub === '-set-usage' || sub === '-set-token') {
    ...
    saveConfig({
      ...
      model:     ctx.modelId,
      mode:      ctx.mode,
      autoTest:  ctx.autoTest,
      autoScan:  ctx.autoScan,
      maxTokens: capped,
    });
...
async function handleWa(text, ctx) {
  ...
  if (args === '--reset') {
    ...
    try {
      ...
      const connPath = path.join(os.homedir(), '.whyWhale', 'connections.json');
      try {
  ...
  if (args.startsWith('owner')) {
    ...
    if (!rest) {
      // Show current owner
      const os       = require('os');
      const connPath = require('path').join(os.homedir(), '.whyWhale', 'connections.json');
      let current    = 'not set';
      try {
        const data = JSON.parse(fs.readFileSync(connPath, 'utf8'));
