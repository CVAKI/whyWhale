...
async function setupModel(ctx) {
  ...
  // Drop saved modelId if it is no longer in the available list
...
// ─── askConnections — optional first-run connection setup ─────────────────────
// Called once after the AI connection test. Asks if the user wants to link
// messaging integrations like WhatsApp. Skipped if connections already exist.
async function askConnections(ctx) {
  ...
  console.log('');
  console.log('  ' + tl('◈') + ' ' + ab('Would you like to connect messaging apps to whyWhale?'));
  console.log('  ' + ab('  (WhatsApp, Telegram, Discord — type ') + sd('/connection') + ab(' any time to set these up)'));
  console.log('');

  const ch = await ctx.ask(cr('  ❯ ') + ab('Connect now? [y/N]: '));
...
async function runSetup(ctx) {
  ...
  // Only ask about connections during first-time setup, not on every launch.
