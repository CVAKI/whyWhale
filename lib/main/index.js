...
async function autoStartConnections(ctx) {
  ...
  if (!fs.existsSync(credPath)) {
    // No Baileys session yet — let it start anyway, it will show a QR to scan once
    console.log('  ' + '\x1b[38;5;157m' + '  (First-time Baileys auth — scan the QR below to activate messaging)' + '\x1b[0m');
  }
  ...
  // ── CHANGE 1: also grab sendFarewellMessage ────────────────────────────────
...
async function main() {
  ...
  // ── CHANGE 2: async exit handler sends farewell before quitting ───────────
