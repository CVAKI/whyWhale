...
function parseFileBlocks(text) {
  ...
  while ((m=re.exec(text))!==null) {
    blocks.push({relPath:m[1].trim(), content:m[2].replace(/\r\n/g,'\n')});
  }
  ...
  while ((m=re2.exec(text))!==null) {
    ...
    blocks.push({relPath:m[1].trim(), content});
  }
  if (blocks.length) return blocks;
...
function applyFileBlocks(blocks) {
  return blocks.map(blk=>{
    try {
      const full=writeFileSafe(blk.relPath,blk.content);
      return {ok:true,path:path.relative(CWD(),full),full,lines:blk.content.split('\n').length};
    } catch(err) { return {ok:false,path:blk.relPath,err:err.message}; }
  });
}
...
module.exports = {
  ...
  parseFileBlocks, applyFileBlocks, printFileResults,
